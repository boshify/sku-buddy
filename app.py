...
if master_file:
    ...
    if master_df is not None:
        master_df.columns = pd.io.parsers.DataFrame({'names': master_df.columns}).columns.get_level_values(0)
...
if supplier_source == "Upload from Computer":
    ...
    if supplier_file:
        ...
        if supplier_df is not None:
            supplier_df.columns = pd.io.parsers.DataFrame({'names': supplier_df.columns}).columns.get_level_values(0)
